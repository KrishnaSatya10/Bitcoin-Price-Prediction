...
def similarity(a, b):
    similarity_score = np.dot(
        (a - np.mean(a)), (b-np.mean(b))) / (len(a)*np.std(a)*np.std(b))
...
def computeDelta(wt, X, Xi):
    ...
    xi_dataset = Xi.iloc[:, :-1]
    yi = Xi.iloc[:, -1]

    xi_dataset['sim_sum'] = xi_dataset.apply(
        lambda x_i: np.exp(wt * similarity(x, x_i)), axis=1)
    xi_dataset['weighted_sim_sum'] = xi_dataset['sim_sum'] * yi

    numerator = sum(xi_dataset['weighted_sim_sum'])
    denominator = sum(xi_dataset['sim_sum'])
...
# YOUR CODE HERE
model = smf.ols('deltaP ~ deltaP90 + deltaP180 + deltaP360',
                data=trainData).fit()
